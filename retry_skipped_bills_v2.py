...
def retry_skipped_bills():
    ...
    accepted_types = ['s', 'hr', 'sjres', 'hjres']
    
    try:
        with sqlite3.connect(utils.DB_PATH) as conn:
            c = conn.cursor()
            c.execute('SELECT title, congress, bill_type, number FROM skipped_bills')
            ...
            for bill in skipped_bills:
                title, congress, bill_type, number = bill
                ...
                if bill_data['text']:
                    summary = utils.summarize_text(bill_data['text'], title, bill_data['status'], congress, bill_type, number)
                ...
                c.execute("DELETE FROM skipped_bills WHERE title=? AND congress=? AND bill_type=? AND number=?", 
                          (title, congress, bill_type, number))
                conn.commit()

                time.sleep(5)
    except Exception as e:
        logging.error(f"Error in retry_skipped_bills: {str(e)}")
        raise
